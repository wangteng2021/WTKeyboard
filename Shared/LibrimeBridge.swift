...
final class LibrimeBridge: RimeNativeBridge {
    ...
    private typealias SearchFunction = @convention(c) (UnsafePointer<CChar>, Int32, UnsafeMutableRawPointer?) -> Int32
    private typealias CleanupFunction = @convention(c) (UnsafeMutableRawPointer?) -> Void
    ...
    func search(for input: String, limit: Int) -> [String] {
        ...
        return queue.sync {
            ...
            return withUnsafeMutablePointer(to: &buffer) { pointer -> [String] in
                let rawPointer = UnsafeMutableRawPointer(pointer)
                let status = input.withCString { cString -> Int32 in
                    search(cString, Int32(limit), rawPointer)
                }
                guard status == 0 else {
                    cleanup(rawPointer)
                    return []
                }
                let results = pointer.pointee.toCandidates()
                cleanup(rawPointer)
                return results
            }
        }
